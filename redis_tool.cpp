...
int RedisTool::Connect()
{
    ...
    {
        get_redis_toolLogs()->error("Connect():redisConnectWithTimeout error:{}", pRedisContext->errstr);
        fprintf(stderr, "Connect():redisConnectWithTimeout error:%s\n", pRedisContext->errstr);
        redisFree(pRedisContext);
        pRedisContext = NULL;
        pthread_mutex_unlock(&redis_tool_mutex); // unlock
        return -1;
    }
    fprintf(stdout, "Connect():Connect complete! address:%s port:%d\n", address.c_str(), port);
